...
def export_articles(
    ...
    """
    Export articles as a CSV string based on pagination, filtering, and sorting.
    """
    query = db.query(Article)
    ...
    if sort_field:
        sort_attr = getattr(Article, sort_field, None)
        if sort_attr:
            if sort_order == -1:
    ...
    for article in articles:
        writer.writerow([
            article.id,
            article.wp_id,
            json.dumps(article.get_categories_id_list()),
            article.title,
            article.slug,
            article.author_id,
            article.status,
            json.dumps(article.get_seo_keywords()),
            article.meta_title,
            article.meta_description,
            article.main_image_url,
            article.main_image_wp_id,
            article.buyers_guide_image_url,
            article.buyers_guide_image_wp_id,
            json.dumps(article.get_products_id_list()),
            article.introduction,
            article.buyers_guide,
            json.dumps(article.get_faqs()),
            article.conclusion
        ])
