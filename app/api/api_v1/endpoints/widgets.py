...
async def generate_product_content(
    ...
    """
    try:
        result = await generate_product_widget(db=db, product_id=product_id)
        return result
...
async def generate_article_content(
    ...
    """
    try:
        result = await generate_article_widget(db=db, article_id=article_id, publish_to_wp=publish_to_wp)
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
